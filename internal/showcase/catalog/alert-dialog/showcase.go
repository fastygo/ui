...
func previewDefault(ctx context.Context, w io.Writer) error {
	return showcaseutil.RenderWithChildren(ctx, w, ui.Box(ui.BoxProps{Class: "max-w-sm rounded-lg border border-border bg-card p-4"}), func(ctx context.Context, w io.Writer) error {
		if err := ui.Title(ui.TitleProps{Order: 3, Class: "text-base font-semibold"}, "Are you sure?").Render(ctx, w); err != nil {
			return err
		}
		if err := ui.Text(ui.TextProps{Class: "text-sm text-muted-foreground"}, "This action cannot be undone.").Render(ctx, w); err != nil {
			return err
		}
		return ui.Group(ui.GroupProps{Class: "mt-4 flex justify-end gap-2"}).Render(templ.WithChildren(ctx, showcaseutil.Child(func(ctx context.Context, w io.Writer) error {
			if err := showcaseutil.Button(ui.ButtonProps{Variant: "outline"}, "Cancel").Render(ctx, w); err != nil {
				return err
			}
			return showcaseutil.Button(ui.ButtonProps{Variant: "destructive"}, "Delete").Render(ctx, w)
		})), w)
	})
}
